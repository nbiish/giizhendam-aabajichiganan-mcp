...
var Protocol = class {
  ...
  _onrequest(request, extra) {
    ...
    Promise.resolve().then(() => handler(request, fullExtra)).then((result) => {
      ...
      return (_a4 = this._transport) === null || _a4 === void 0 ? void 0 : _a4.send({
        result,
        jsonrpc: "2.0",
        id: request.id
      });
...
var orchestrateParamsSchema = external_exports.object({
  prompt_text: external_exports.string().max(1e4).describe("Primary task prompt passed into each configured CLI agent.")
});
server.tool(
  "orchestrate_agents",
  ...
  async (params) => {
    ...
    const synthFile = import_path33.default.join(AGENT_OUTPUT_DIR, `synthesis_${Date.now()}.md`);
    try {
...
async function openaiChat(model, messages, options) {
  try {
    const openaiMessages = messages.map((msg, idx) => {
...
async function loadAgents() {
  ...
  const llmsPath = import_path33.default.join(process.cwd(), "llms.txt");
  try {
...
async function runAgentCommand(agent, prompt, outDir) {
  ...
  const outFile = import_path33.default.join(outDir, `${agent.name.toLowerCase().replace(/[^a-z0-9_-]/gi, "_")}_${Date.now()}.txt`);
    ...
    child.on("close", async (code) => {
      if (chunks.length > 0) {
        ...
        if (!import_fs.default.existsSync(outFile)) {
          try {
            await import_promises.default.writeFile(outFile, "Agent executed but produced no stdout. Check side effects in output directory.");
          } catch (e) {
          }
        }
      }
...
async function loadAgentPrompts() {
  const agentsDir = import_path33.default.join(process.cwd(), "agents");
  const prompts = {};
  try {
    const files = await import_promises.default.readdir(agentsDir);
    ...
    for (const file of mdFiles) {
      ...
      try {
        const content = await import_promises.default.readFile(import_path33.default.join(agentsDir, file), "utf8");
...
var EXPERT_PROMPTS = {};
var financeExpertsParamsSchema = external_exports.object({
...
var financeExpertsOutputMetaSchema = external_exports.object({
  ...
  expertOutputsDir: external_exports.string().optional().describe("Directory containing individual expert output files"),
  fileSaveSuccess: external_exports.boolean().optional(),
  expertsProcessed: external_exports.array(external_exports.string()),
  expertFilesCount: external_exports.number().optional().describe("Number of expert output files created"),
  ragConsolidationUsed: external_exports.boolean().optional().describe("Whether Gemini File Search RAG was used for consolidation"),
  recommendedPrompt: external_exports.string().optional().describe("The orchestrator prompt suggested by the expert deliberation for CLI tools execution."),
  apiErrors: external_exports.array(external_exports.object({ expert: external_exports.string(), error: external_exports.string() })).optional(),
  errorType: external_exports.string().optional().describe("Error: 'InitializationError', 'FileSystemError', 'ApiError', 'ConfigurationError'.")
});
server.tool(
  "finance_experts",
  ...
  async (params) => {
    ...
    const expertOutputsDir = import_path33.default.join(outputDirFinance, `expert_outputs_${Date.now()}`);
    await import_promises.default.mkdir(expertOutputsDir, { recursive: true });
    const expertFiles = [];
    await Promise.allSettled(expertsToProcess.map(async (expert) => {
      ...
      try {
        ...
        const responseText = await openaiChat(
          ...
          { maxTokens: 8e3, temperature: 0.7 }
          // Increased limit for comprehensive output
        );
        const expertFileName = `${expert.toLowerCase().replace(/[^a-z0-9_-]/gi, "_")}_${Date.now()}.md`;
        const expertFilePath = import_path33.default.join(expertOutputsDir, expertFileName);
...
${responseText}`;
        await import_promises.default.writeFile(expertFilePath, expertContent);
    ...
    try {
      ...
      for (const filePath of expertFiles) {
        try {
          const fileContent = await import_promises.default.readFile(filePath, "utf8");
          const fileName = import_path33.default.basename(filePath);
          ragFiles.push({
            inlineData: {
...
[Overall confidence score (1-10) and risk factors]`;
      ...
      consolidatedAnalysis = await openaiChat(
        ...
        {
          maxTokens: 16e3,
          // Increased to allow very comprehensive analysis
          temperature: 0.7,
      ...
      try {
        const allExpertText = expertResponses.map(
          ({ expert, response }) => `## ${expert}'s Analysis
...
Provide consolidated analysis and recommended orchestrator prompt.`;
        orchestratorDeliberation = await openaiChat(
          ...
          { maxTokens: 16e3, temperature: 0.7 }
        );
...
**Analysis Date:** ${(/* @__PURE__ */ new Date()).toISOString()}
...
`;
    ...
    const outputFilePath = import_path33.default.join(outputDirFinance, outputFilename);
...
var ceoBoardParamsSchema = external_exports.object({
  topic: external_exports.string().max(2e3).describe("Topic for board discussion (e.g., 'Q3 Strategy Review')."),
  roles: external_exports.array(external_exports.string().max(100)).optional().describe("Optional board roles. Uses standard roles if not provided."),
  output_filename: external_exports.string().optional().describe("Optional filename. Defaults to sanitized topic.")
});
var boardSimulationOutputMetaSchema = external_exports.object({
  success: external_exports.boolean(),
  outputFilePath: external_exports.string().optional(),
  fileSaveSuccess: external_exports.boolean().optional(),
  recommendedPrompt: external_exports.string().optional().describe("The orchestrator prompt suggested by the board deliberation for user execution."),
  apiError: external_exports.string().optional(),
  errorType: external_exports.string().optional().describe("Error: 'InitializationError', 'FileSystemError', 'ApiError', 'ConfigurationError'.")
});
server.tool(
  "ceo_and_board",
...
[A number from 1-10 for the recommendation.]`;
    ...
    const outputFilename = `${safeFilenameBase}_${Date.now()}.md`;
    const outputFilePath = import_path33.default.join(outputDirBoard, outputFilename);
    try {
