...
server.tool(
  ...
  "Generates primary and verification aider CLI command strings for agentic 'fire-and-forget' tasks. The assistant should run these using a terminal execution tool.",
...
server.tool(
    ...
    listProvidersParamsSchema.shape,
    async (params) => {
...
const FINANCE_EXPERT_PROMPTS: Record<typeof FINANCE_EXPERT_PERSONAS[number], string> = {
...
server.tool(
    "finance_experts",
    "Consult various financial expert agent personas for analysis or insights.",
    ...
    async (params) => {
        ...
        const expertsToConsult: ReadonlyArray<typeof FINANCE_EXPERT_PERSONAS[number]> = 
            (params.experts && params.experts.length > 0)
            ? params.experts
            : ["Warren Buffett"]; // Default to Warren Buffett if none specified

        const consultationPrompts: { expert: typeof FINANCE_EXPERT_PERSONAS[number]; combinedPrompt: string }[] = [];
        const errors: { expert: typeof FINANCE_EXPERT_PERSONAS[number]; error: string }[] = [];
